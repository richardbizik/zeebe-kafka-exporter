...
public class RawRecordsConfigParser implements ConfigParser<RawRecordsConfig, RecordsConfig> {
  ...
  public RecordsConfig parse(final RawRecordsConfig config) {
    ...
    Optional.ofNullable(config.messageBatch)
        .map(recordConfigParser::parse)
        .ifPresent(c -> typeMap.put(ValueType.MESSAGE_BATCH, c));
    Optional.ofNullable(config.form)
        .map(recordConfigParser::parse)
        .ifPresent(c -> typeMap.put(ValueType.FORM, c));
    Optional.ofNullable(config.userTask)
        .map(recordConfigParser::parse)
        .ifPresent(c -> typeMap.put(ValueType.USER_TASK, c));
    Optional.ofNullable(config.processInstanceMigration)
        .map(recordConfigParser::parse)
        .ifPresent(c -> typeMap.put(ValueType.PROCESS_INSTANCE_MIGRATION, c));
    Optional.ofNullable(config.compensationSubscription)
        .map(recordConfigParser::parse)
        .ifPresent(c -> typeMap.put(ValueType.COMPENSATION_SUBSCRIPTION, c));
